...
- **Never hardcode a model name.** The roster lives in `config/models.yaml`, validated against OpenRouter's live `/models`, with a top-level `snapshot_date` (one for the file, not one per row) and a pinned provider+quantization tag per model. Model names in this repo's prose are *candidates*, not confirmed facts — the open-weight landscape moves every 6–8 weeks.
...
- **Anything not bought from a real API gets `generations.is_mock = 1`.** Every analysis query filters on it. An unflagged synthetic row is indistinguishable from a $0.0006 purchased one, and by the time that matters the mistake is invisible.
- **Never edit `data/carr.sqlite` without a snapshot.** `scripts/studio.py` takes one automatically into `data/backups/`; anything else destructive must call `carr.db.backup()` first. `generations` cannot be rebuilt without paying again.
