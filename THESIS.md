...
| | |
|---|---|
| **Stage** | **Grid run, and re-audited 2026-09-01.** A hostile-examiner pass over the whole thesis found **twelve real defects**, all free to fix and all now fixed in code and docs — including one that **reverses the sign of the headline** (see the log). Data collection is done; analysis is done and now correct; what remains is **prose**. |
| **Next action** | **Re-read [docs/defence/](docs/defence/00-everything-a-to-z.md) §"what changed" first** — twelve headline numbers moved on 2026-09-01 and the old ones are still in the `.docx`. Then write **Chapter 5** around the *per-model* effect ([docs/learn/36](docs/learn/36-writing-ch5-7.md) gives the structure), not the tier aggregate. Then the structural `.docx` edits (1, 2, 3, 12, and the new 9b/10b/11a/11b/12b/13b in [docs/docx-revisions.md](docs/docx-revisions.md)). **Two free, high-value jobs also owed:** run `runner.reconcile_costs()` over the 1,207 grid rows that still carry an `openrouter_gen_id`, and write the AI-use declaration (§16). Plus full reads of Route-To-Reason, Agent-as-a-Router and HRBench. **$0.** |
...
| **Spend to date** | **$5.25** of **$15.00 loaded** ($9.75 left; self-imposed cap $6.00, never breached). Buying stopped deliberately. ⚠️ **Two numbers, both correct:** `db.summary()` reports **$5.240216** across the 1,373 stored generations — quote that for anything computed from the table. **$5.25** is total account spend, which additionally covers exploratory calls made outside the runner (the day-1 probe, the $0.010978 abort verification, the reasoning-budget tests) that were never written as `generations` rows. |
| **Rows in dataset** | **1,373 generations, 1,280 graded**, across 320 problems × 10 configs. Coverage is uneven **and not at random** — see §11: the run order's `problem_id` tiebreak made the thinking arm almost entirely LeetCode-style. 1,025 no-reasoning calls, 348 thinking. Problem pool **884** |
| **Blocked on** | Nothing. ⚠️ But see the saturation evidence in §11 before choosing the pilot sample |

**Recent log**
- `2026-09-01` — **🔴 A HOSTILE-EXAMINER AUDIT FOUND TWELVE REAL DEFECTS. All free to fix; all fixed. $0 spent.** Twenty adversarial passes over the thesis, each fact-checked against `data/carr.sqlite`, produced **419 viva questions (111 rated fatal)** in [docs/defence/](docs/defence/00-everything-a-to-z.md) — and, far more importantly, **broke several headline claims.** **(1) 🔴 THE HEADLINE IS PER MODEL, NOT PER TIER.** "Thinking helps on hard problems, +29.3" averages **+52.9 on `deepseek-v4-flash`** (29.2%→82.1%) against **−18.2 on `qwen3.5-9b`** (22.1%→3.8%). Reasoning is a property of the **(model, tier) pair**. Two mechanisms, now separated: **non-termination** (`qwen3.5-9b|high` censors **81%** of hard calls) and **genuine degradation** — on MBPP+ it censors **0%**, reasons 368 tokens, terminates, and still falls **16/20 → 10/18 on the same 20 problems**, every failure an assertion error. That second one is overthinking measured directly with truncation ruled out, and nothing in the thesis had isolated it. **(2) 🔴 A FREE ABORT THRESHOLD DOES EXIST — for two of five models.** `qwen3.5-9b` at T=10,000 keeps **46/46** solutions and saves **88%**; `qwen3.6-35b` at 16,000 keeps 43/43. It appears exactly where reasoning was not earning its keep, and vanishes where it was. The roster-level refutation stands. **(3) The effort arms sat different exams.** `runner.plan` breaks ties on `problem_id` *as a string*; LeetCode ids are numeric and AtCoder ids start with letters, so all 125 functional problems ran before any of the 217 stdin ones and the thinking arm hit the cap inside that prefix. Hard tier: `off` is 348 stdin/114 functional, `high` is **8/110**. Style-matched: hard **+25.7**, medium **+28.9** (*larger*). The 60-problem frontier set is **51 functional / 2 stdin**. **(4) The 81/98/141 split is coverage-dependent** — 206 of 320 problems saw only 2–3 configs; **94 of the 98 "solved by nothing" were never shown a reasoning config**; at ≥6 configs **82% discriminate**. Per-tier saturation survives (it is a pass rate); "76% carries no signal" does not. **(5) 76% of the waste is one model** (`qwen3.5-9b`, 37 of 99 thinking calls). **(6) Costs are computed, not billed** — `cost_actual_usd` is on **139 of 1,373 rows, all the pilot**; the 1,224-call grid was never reconciled. **(7) 305× is not like-for-like** — 228× on the 22 shared problems, **65×** on an identical six-config exam. **(8) Two of the router's three "free features" are benchmark metadata** — `difficulty` is LCB's own label and `n_tests` is the hidden grading suite; only `prompt_chars` is a prompt feature. This falsifies the proposal's positioning independently of the router failing. **(9) The reasoning traces were never stored** — `raw_response` is `message.content`; `message.reasoning` is never read, so `reasoning_tokens` is a **vendor annotation** nothing checks. **(10) `pro|high` vs `flash|high` is an exact tie**, 64/68 each, on the problems both sat — quote the tie, not "3 points less". **(11) "15% of spend" had the wrong denominator** (15% of the $3.62 reasoning arm; **11%** of the $5.24 total), fixed in 24 places. **(12) Stale facts corrected:** §9's roster table carried `/models` headline prices contradicting the pinned prices actually used, and claimed 9 configs; "210 canonical solutions" is 3 in the standing suite; "16 tests on the cap" is 19; the `$0.010978` abort baseline is not reproducible; `snapshot_date` is one per file, not per row; MBPP+'s 378 is not "500 minus broken" (ids span 2–809); the 58× first-cell spread reproduces as **44×** at pinned prices. **New:** `analysis.within_model_effect / waste_by_model / discrimination_by_coverage / style_composition / style_matched_effect / abort_by_model`, all printed by `results.py`; **figure 05-effect-by-model.png**; **132 tests** (was 119). **§16 added: there is no AI-use declaration anywhere, and 37 commits are AI co-authored** — draft wording provided; this is the highest-risk omission at submission.
- `2026-07-28` — **[docs/learn/](docs/learn/00-index.md) — a 40-lesson course from zero to a written thesis. $0 spent.** START-HERE teaches the machine in an hour; this teaches everything, assuming no prior knowledge of LLMs, tokens, SQL, statistics, or what a thesis even is. Four parts: **foundations** (01–10: what a token is, why reasoning tokens are billed and invisible, providers and quantization, benchmarks and grading, SQL, denominators, the bootstrap, Pareto/hull/oracle); **the science** (11–20: one lesson per finding, each with its numbers, caveats and the hostile question it must survive); **the code** (21–31: every module, and the bug each rule prevents); **the writing** (32–40: what examiners check, chapter-by-chapter drafting, figures and citations, the 14 `.docx` edits, and twenty defence questions with answers). Every lesson has something free to run and self-check questions with answers. **Three number inconsistencies found while writing it and recorded in lesson 38:** `$5.24` (generations, verified from `db.summary()`) vs `$5.25` (total spend) are used interchangeably in places; the superseded saturation counts 84/116/120 still appear alongside the current **81/98/141**; and `docx-revisions.md` item 7 quotes LiveCodeBench's **112 stdin / 63 functional** split, which is v6-only — the pool actually used is v5+v6, **217 / 125 of 342**. All headline denominators re-verified against the live database: 81/141/98, 107 paired, 6 configs × 60 problems, 884 pool, 1,373 generations, 1,280 graded, $5.240216. README's "32 tests" corrected to **119**.
- `2026-07-27` — **[docs/START-HERE.md](docs/START-HERE.md) — learn the project from zero in ~1 hour. $0 spent.** Seven steps, each with something to run and what you should see: the one-sentence question, the one-row mental model, the four tables and which one cost money, the code in dependency order, `results.py` read top to bottom, the three things that surprise, and the rules that keep it honest. `codebase-tour.md` is now marked stale at the top — it predates `stats.py`, `figures.py`, `router.py`, `livecodebench.py` and five scripts, and several of its "NOT BUILT YET" notes are wrong. README points newcomers at START-HERE first.
...
- `2026-07-27` — **Confidence intervals and CPC/TPC. $0 spent.** `carr/stats.py` — a seeded percentile bootstrap, stdlib only, no scipy needed. Every headline number now carries a 95% interval and an explicit denominator. **(1) The comparability trap is closed.** The grid is unbalanced (off configs 320 problems, high 51–104, kimi 16–23), so a per-config cost number over *that config's own* problems compares models on different exams. `paired_problems()` gives the honest denominator: **107 of 320** problems have both effort arms graded, and it is printed next to every result. **(2) CPC and TPC now exist** — §13 calls CPC the headline economic metric and nothing computed it. Cheapest per correct answer is **`flash|off` at $0.00021**; dearest is **`kimi|high` at $0.06320 — a 305× spread**. Both are ratios of sums, bootstrapped by resampling *problems*, never by averaging per-problem ratios (the §10.2 error, now pinned by a test). **(3) The intervals immediately earned their keep: 5 adjacent config pairs have overlapping CPC intervals**, so their ordering is *not* established despite distinct point estimates — exactly the "frontier difference within noise" §10.2 warns about. **(4) The abort curve's intervals are wide**: keeping 88% of solutions at a 16k threshold has a CI of [82, 92]%, and the 49% saving is [35, 61]%. Bootstrap resamples problems, not cells, because two cells on one problem are not independent.
- `2026-07-27` — **🔴 THE GRID REFUTED THE PILOT'S HEADLINE. $5.25 spent, buying stopped.** 1,373 generations, 1,280 graded, 320 problems. **(1) There is no free abort threshold.** The pilot's "abort at 10,000 reasoning tokens keeps every solved problem and saves 44%" was an artefact of the 16k ceiling: truncated calls *cannot* succeed, so the ceiling manufactured the cliff. At 48k, **56 calls above 10,000 reasoning tokens succeeded**. Pass rate declines as a gradient, not a cliff — **83.8%** under 10k, **55.2%** at 10–20k, **39.3%** above 20k. What survives is an honest tradeoff curve: abort at 16,000 keeps **212/242 solutions (88%) for half the cost**. `best_threshold()` correctly returns None. **(2) The pilot's other headline also reversed**: thinking is *better* than not-thinking on both discriminating tiers (hard 31.3% vs 24.9%, medium 64.3% vs 53.3%), where the 15-problem pilot said the opposite. **(3) Only 120 of 320 problems discriminate** — 84 solved by everything, 116 by nothing. **(4) Censoring remains at the curve's right end**: 26.3% of hard thinking calls still truncate at 48k, so the ">20k" band's 39.3% is a floor, not an estimate.
- `2026-07-26` — **⚠️ `max_tokens` was censoring the headline, not just causing failures.** At the pilot's 16k ceiling, **43.8% of LCB-hard and 36.4% of LCB-medium thinking calls were cut off** and returned nothing. A censored call *cannot* succeed, so "nothing above 10,000 reasoning tokens ever succeeded" is partly an artefact of the ceiling rather than a property of the models — which puts the 44%-saving headline in question until it is re-measured. Ceiling raised **32k → 48k** (~4× any threshold worth considering; every pinned endpoint allows 81,920+). **Not removed**, which was the tempting fix: without a ceiling the worst case is bounded only by context — kimi at 262,144 tokens is $0.89 for one call — and the cost cap computes its reservation *from* `max_tokens`, so it would have nothing to bound. `analysis.censoring()` now reports the rate next to every result so this can never hide again.
...
| **matplotlib** ✅ | Figures | Exports PNG for pasting into Word. The only dependency added after Day 1 |
...
| 🟨 | 8–9 | Formal section + figure generation | **Figures ✅** (5 PNGs, `carr/figures.py`). Formal section: §10.1 hull/oracle built; §10.2 gap decomposition still ⬜ |
...
├── docs/
│   ├── START-HERE.md        ✅ # ← the project in an hour, seven runnable steps
│   ├── learn/               ✅ # ← THE COURSE: 00-index.md + 40 lessons, zero → written thesis
│   ├── defence/             ✅ # ← VIVA PREP: A-to-Z from zero, the question bank, the cheatsheet
│   ├── research-framing.md  ✅ # the thesis stated as research, not code
│   ├── docx-revisions.md    ✅ # the 14 edits owed to the proposal
│   ├── codebase-tour.md     ✅ # longer file-by-file walk. ⚠️ written early, partly stale
│   ├── data-spec.md         ✅ # exact send/store contract
│   ├── advisor-update.md    ✅ # the one-pager to send
...
│   ├── figures.py           ✅ # 5 PNGs → data/figures/ (gitignored)
...
└── tests/
    ...
    ├── test_runner.py       ✅ # 19 tests — THE COST CAP. Proves it aborts
...
### Roster — 3 families, 5 open-weight models (no closed reference was run)
...
⚠️ **Corrected 2026-09-01.** This table previously carried the `/models`
*headline* prices — which is the very trap the pinning finding is about, since
the headline is the cheapest provider's price and not the one we contracted. The
prices below are the **pinned endpoint's**, copied from
[`config/models.yaml`](config/models.yaml), which is authoritative. Superseded
headline figures are struck through so the size of the gap is visible.

| Model slug | Family | Pinned provider | in $/M | out $/M |
|---|---|---|---|---|
| `qwen/qwen3.5-9b` | qwen | `deepinfra/bf16` | 0.100 | **0.150** |
| `deepseek/deepseek-v4-flash` | deepseek | `baidu/fp8` | 0.091 | **0.182** ~~0.1876~~ |
| `qwen/qwen3.6-35b-a3b` | qwen | `akashml/fp8` | 0.140 | **1.000** |
| `deepseek/deepseek-v4-pro` | deepseek | `baidu/fp8` | 0.625 | **1.251** ~~0.870~~ |
| `moonshotai/kimi-k2.6` | moonshot | `siliconflow/fp8` (**RQ5 held-out**, subset) | 0.770 | **3.400** ~~2.720~~ |

**5 models × 2 efforts (off / high) = 10 configs**, one of which — kimi, both
efforts — is held out and runs on a subset. Pairing each model with *itself* at
two effort levels keeps the effort axis clean: within a pair, only the thinking
mode changes. *(Superseded: ~~4 models × 2 efforts = 8 configs, + 1 held-out = 9~~
— kimi was given both efforts on 2026-07-26, see §12.)*

⚠️ **There is no closed-model reference.** The heading above once promised one;
none was ever run. Say so rather than letting an examiner find it.
...
- **Output prices span 18×** across the roster ($0.15 → $2.72). Combined with thinking's token multiplier — **measured at 4.3×** across 1,355 graded calls (mean completion 3,166 `off` vs 13,604 `high`), not the ~10× estimated before any data — the most expensive config costs roughly 80× the cheapest. That spread *is* the thesis. *(Superseded: ~~~10× multiplier, ~180×~~ — an estimate made before the grid ran.)*
...
- **MBPP+** — sampled from the **378 available**. ⚠️ *Not* "500 minus the broken ones": the 378 span task ids **2–809**, of which only **224** fall inside the canonical 11–510 test split and **154** sit outside it. EvalPlus filters and augments across a wider range than the 500 people quote, so the proposal's "500" is the wrong base as well as the wrong count
- **LiveCodeBench** — ✅ **v5+v6 loaded: 342 problems** (154 hard, 104 medium, 84 easy; **217 stdin-style AtCoder, 125 function-style LeetCode**). This is the hard tier that stops RQ4 from degenerating. *(Superseded: the first load was v6 alone — ~~175 problems, 80/52/43, 112 stdin / 63 functional~~. Quote the v5+v6 figures; the v6-only split is still miscited in places.)*
...
selection: HumanEval+ is all 164, MBPP+ is all 378 (ids 2–809; 224 inside the
canonical 11–510 split, 154 outside it — not the 500-split minus breakages
...
1. **Hard cap that aborts.** ✅ Built and tested (**19** tests in `tests/test_runner.py`, three named directly on the cap abort). `config/experiment.yaml` holds `abort_at_usd: 6.00`; `carr/runner.py` refuses any call whose *worst case* would push **lifetime** spend past it, and a per-run `run_headroom` keeps a single run near its own estimate. It stops — it does not warn and continue.
...
> identical outcome. *(Those are pre-pinning prices. Re-run against the current
> database and the same cell gives **44×**, $0.000106 to $0.004630, still
> 10/10 PASS: pinning re-priced several models and costs were later reconciled
...
| **🔴 "No free abort threshold" is a POOLED claim, and it is false for two of five models** | `best_threshold()` returns None over the roster. Per model: **`qwen3.5-9b` — abort at 10,000 reasoning tokens keeps all 46 of its solutions and saves 88%** of its thinking spend; `qwen3.6-35b-a3b` — 16,000, keeps 43/43, saves 13%; `flash`, `pro` and `kimi` — none, every threshold costs a solution. **The pattern is coherent:** the free threshold exists exactly where reasoning was *not* earning its keep (`qwen3.5-9b`'s delta is negative on three tiers and 37% of its calls return nothing), and vanishes where it was (`flash` gains +52.9 on hard *by* thinking longer). **This refines the refutation without undoing it** — the pilot's roster-level free threshold remains false, and the 16k ceiling really did manufacture part of it. `analysis.abort_by_model()`, printed by `results.py` |
| **⚠️ "Log the actual temperature" was never implemented** | §9 says *"Log the actual temperature — some thinking endpoints silently override it."* `generations.temperature_sent` records what we **sent**: 0.0 on all 1,373 rows, a constant, so it carries no information about whether the endpoint honoured it. Nothing anywhere records the temperature actually applied. And with **n=1 per cell** there is no way to detect nondeterminism empirically either. **Determinism is an assumption here, not a verified property** — say so when asked how you know temperature 0 was honoured |
| **🔴 The reasoning traces themselves were never stored — only the vendor's count** | `providers/openrouter.py` sets `raw_response = choice.message.content`, the *visible answer*. `choice.message.reasoning` — which OpenRouter returns for these models — is **never read**, and there is no `reasoning_text` column. So the quantity this thesis is about was never observed directly: gen 1333 burned **73,037 reasoning tokens** and what survives is an 18KB block of code and an integer. **Three consequences.** (1) **`reasoning_tokens` is a vendor annotation**, not a measurement — every cost, waste and abort number rests on a number the provider asserts, with no independent check anywhere in the pipeline. (2) **The overthinking claim cannot be inspected**: nobody can say what the 49 wasted calls were *doing* for a mean of 29,584 tokens — looping, exploring, or restating. (3) **It is not recoverable offline.** Unlike an extraction bug, which `raw_response` makes free to fix, this needs re-buying. **Say it as a limitation and as the first thing you would change.** |
| **⚠️ The $0.010978 abort-comparison baseline is not reproducible** | The **$0.00 cancellation is solid** — verified on two providers, no billing record afterwards. The *comparison* figure is not: the abort test was an exploratory call made outside the runner and never written to `generations`, and the only row carrying that exact cost is `qwen3.6-35b-a3b\|high` on `LiveCodeBench/3697` (gen 141) — **a different model from the `deepseek-v4-pro\|high` the log attributes the test to.** Do not quote "the same cell run to completion cost $0.010978" as a datum. **Reproducible replacement:** a completed `pro\|high` call averages **$0.012165** (n=73, max $0.060253) and `kimi\|high` **$0.057706** (n=23); an abort saves all of it, which is the actual point. Corrected in `carr/analysis.py` and lesson 16, 2026-09-01 |
| **🔴 Two of the router's three "free features" are benchmark metadata, not prompt features** | `rank` comes from `problems.difficulty` — **LiveCodeBench's own hardness label** (and for HE+/MBPP+ it degrades to the benchmark name); `n_tests` is base+plus, i.e. **the size of the hidden grading suite**, not knowable before grading. Only `prompt_chars` is computable from an arriving prompt. So the proposal's positioning — routing on "cheap, non-LLM structural and lexical features" of the incoming prompt — **is not what was implemented**, and that holds whether or not the router worked. It also weakens the decomposition: "feature insufficiency 0.0" means *these* features, two thirds of them labels a deployment would not have, suffice. **What survives untouched:** the oracle, the hull and the 13.8-point headroom, none of which use these features. Corrected in `carr/router.py`'s docstring 2026-09-01, which had claimed the features were "only what the problem statement already tells you" |
| **⚠️ The "15% of reasoning spend buys nothing" finding is 76% one model** | Wasted (billed, no answer) as a share of each model's thinking calls: **`qwen3.5-9b` 37/99 = 37.4%**, `qwen3.6-35b` 7/74 = 9.5%, `flash` 3/73 = 4.1%, `pro` 2/72 = 2.8%, `kimi` 0/22. So the aggregate rate is the wrong number to give a practitioner. Non-termination is a property of the **small model**, not of reasoning — the same story `within_model_effect()` tells about the sign reversal. `analysis.waste_by_model()`, printed by `results.py` |
| **🔴 Only 10% of rows were reconciled against the actual bill** | `cost_actual_usd` is populated on **139 of 1,373 rows, all of them the 2026-07-25 pilot**. The **1,224-call grid was never reconciled**, so every headline dollar is `cost_computed_usd` = tokens × the price table. Defensible because the grid pinned one provider with `allow_fallbacks:false`, making the endpoint price contractual — but **unverified**, and on the 139 rows where it could be checked **actual ran 1.354× computed** (2.16–2.51× on `deepseek-v4-pro`). That gap is best read as evidence *for* the pinning finding, since the pilot pre-dates pinning; say it as an argument, not a fact. **Fixable for $0:** `runner.reconcile_costs()` exists and **1,207 of 1,224 grid rows still hold an `openrouter_gen_id`**; `GET /generation` is free. Risk: records five weeks old may have expired, in which case that is itself the reportable limitation. **Docs that claimed "every call priced from the actual bill" corrected 2026-09-01** |
| **🔴 The headline effect is per MODEL, and the aggregate hides a sign reversal** | "Thinking helps on hard problems, +29.3" averages **+52.9 on `deepseek-v4-flash`** (29.2%→82.1%, n=154/28) against **−18.2 on `qwen3.5-9b`** (22.1%→3.8%, n=145/26). Reasoning is a property of the **(model, tier) pair**, not of the tier. Two mechanisms, and they need separate sentences: **non-termination** — `qwen3.5-9b|high` censors **81%** of hard calls, returns no code on 77%, mean 24,826 reasoning tokens, where `flash|high` censors 11% on the same problems; and **genuine degradation** — on MBPP+ the same model censors **0%**, reasons 368 tokens, terminates, and still falls 16/20 → 10/18 on the **same 20 problems**, every failure with extracted code failing on assertions. The second is overthinking measured directly with truncation ruled out, and it is a cleaner result than the aggregate. Also: the easy-benchmark rows are **model-confounded** (the `high` arm is 62% `qwen3.5-9b` on MBPP+, 58% on HumanEval+, vs ~25% in `off`). **Mitigation is free:** `analysis.within_model_effect()`, printed by `results.py` |
| **🔴 The 81/98/141 problem split is coverage-dependent** | Unanimity is trivially easier to reach with fewer voters, and coverage runs 2–10 configurations per problem: **206 of 320 problems saw only two or three**, nearly always the cheap `off` tier. **94 of the 98 "solved by nothing" were never attempted by any reasoning-enabled config.** Recomputed at real coverage: ≥1 thinking config → 71/108 (66%) discriminate, 4 unsolvable; ≥6 configs → 60/73 (**82%**) discriminate, 2 unsolvable. **Mitigation is free:** `analysis.discrimination_by_coverage()`, printed by `results.py`. **What survives:** the per-tier saturation result, because it is a pass rate rather than a unanimity count — HumanEval+/MBPP+/LCB-easy sit at 72–100% either way. **What does not:** the claim that ~76% of the pool carries no routing signal. Drop it rather than defend it |
| **🔴 The effort arms did not sit the same exam — the LCB style confound** | LiveCodeBench ships stdin→stdout (AtCoder) and `Solution`-class (LeetCode) problems, and coverage of the two is wildly uneven: on the hard tier the `off` arm is **348 stdin / 114 functional**, the `high` arm is **8 / 110**. So the raw 24.9% → 54.2% headline mixes a reasoning effect with a composition effect. **The cause is mechanical:** `runner.plan` sorts cells by `(expected_usd, problem_id)`, ties break on the id *as a string*, LeetCode ids are numeric and AtCoder ids start with letters, so all 125 functional problems ran before any of the 217 stdin ones and the thinking arm hit the cap inside that prefix. **Mitigation is free** — `analysis.style_composition()` and `style_matched_effect()` report the within-style comparison, printed by `scripts/results.py`: hard **31.6% → 57.3%** (n=114/110, **+25.7** not +29.3) and medium **49.1% → 78.0%** (n=163/141, **+28.9** not +23.0). The finding survives on both tiers; the wording "more than doubles" does not. Nothing is claimed about stdin problems — that arm is n=8. The frontier subset inherits the skew (51 functional / 2 stdin), so the hull, oracle, 13.8-point headroom and router result describe **function-style** problems |
| **⚠️ The grid is unbalanced, so most per-config numbers are not comparable** | `off` configs have 320 problems each **for three of the five models — `pro|off` ran only 51 and `kimi|off` 16** — while `high` is 73–104 (kimi 23), the held-out model 16–23. A cost-accuracy point computed over a config's *own* problems compares models on different exams. Mitigation: `analysis.paired_problems()` restricts every comparable statistic to the **107 problems with both effort arms graded**, and `scripts/results.py` prints that denominator and the per-config `n` on every row. Balancing the thinking arm would cost ~$1.50 and require raising the $6.00 cap; recorded as a limitation instead |
...
| 2026-07-28 | **A 40-lesson course (`docs/learn/`) rather than expanding START-HERE** | Two different jobs. START-HERE gets a competent reader running the project in an hour and assumes they know what a token, a benchmark and a confidence interval are. The course assumes none of that, and has to teach the science, the code *and* how to write a thesis — which is weeks of material, not an hour's. Merging them would have made the hour-long path unusable. START-HERE now points at the course for anyone who wants the long road |
| 2026-07-28 | **Every lesson carries the caveat next to the claim, never deferred** | The course is also the drafting material for the chapters, and the thesis's rule is that a caveat travels in the same paragraph as the finding (overlapping CPC intervals, 54.2% as a floor, the fitted feature ceiling). A course that taught the headline first and the caveat later would train exactly the writing habit the thesis must avoid |
| 2026-09-01 | **Report the abort threshold per model, not only pooled** | The pooled curve says no threshold is free; two of five models have one, and `qwen3.5-9b`'s saves 88% while losing nothing. Reporting only the pooled version hands a practitioner a rule that is wrong for the models where it matters most. It also sharpens the refutation story rather than weakening it: the roster-level claim stays false, and the reason the per-model one is true is the same reason the roster-level one is not — reasoning pays off for some models and not others |
| 2026-09-01 | **Report grid costs as computed-from-pinned-price, not as billed** | The claim "every call priced from OpenRouter's actual bill" was true of the pilot and false of the grid — 139 of 1,373 rows are reconciled. Correcting the wording costs nothing and removes a claim an examiner could check in one query. Running the reconciliation is the better fix and is free; left as the next action rather than done silently, because it means 1,207 API calls against the user's account |
| 2026-09-01 | **Lead the results chapter with the per-model effect, not the per-tier aggregate** | The aggregate averages +52.9 and −18.2 into +29.3, which is not a finding about difficulty but an artefact of averaging over models that respond oppositely. Reporting per (tier, model) with the censoring share attached also separates the two mechanisms — non-termination (81% censored) from genuine degradation (0% censored, −24.4 points on the same 20 problems) — and the second was not isolated anywhere before. Free: a re-analysis, not a re-purchase |
| 2026-09-01 | **Report the 81/98/141 split at several coverage cuts, and drop the "76% carries no signal" claim** | The split counts unanimity across however many configs happened to run, and 206 of 320 problems saw only two or three. 94 of the 98 "solved by nothing" were never shown to a reasoning-enabled config, so that bucket measures the budget, not the problems. At ≥6 configs, 82% discriminate. The per-tier saturation finding is unaffected because it is a pass rate. Reported at cuts rather than silently restricted, so the dependence is visible instead of chosen |
| 2026-09-01 | **Report the LCB style confound rather than re-buying to fix it** | The two effort arms sat different exams (hard: 348/114 stdin/functional off, 8/110 high), because the runner's `problem_id` tiebreak ran every LeetCode problem before any AtCoder one and the cap fired inside that prefix. Balancing it means buying ~110 hard stdin thinking calls; at `flash|high`'s measured $0.00177/problem that is affordable, but at the roster's thinking configs it is not, and buying would reopen a closed budget. The within-style re-analysis is **free**, it is the comparison that was wanted anyway, and the finding survives it (+25.7 on hard, +28.9 on medium). Made reproducible in `analysis.style_composition()` / `style_matched_effect()` and printed by `results.py`, so it can never hide again — the same treatment `censoring()` got |
| 2026-09-01 | **Every headline ratio must name the problem set both sides sat** | The 305× CPC spread compares rows with n=107 and n=22 over *different* problems. On the 22 they share it is 228×; over the six configs that sat the identical 60-problem exam it is 65×. All three are large, so the claim survives, but quoting one without its denominator is the easiest catch an examiner has. `results.py` now prints all three |
...
## §16. Use of AI assistance — the declaration owed to the submission

⚠️ **This section is a gap being closed, not a finished statement.** As of
2026-09-01 the repository contained **no AI-use declaration anywhere**, while
**37 of its commits carry `Co-Authored-By: Claude`** and
[CLAUDE.md](CLAUDE.md) is a standing instruction file for an AI assistant. Most
institutions in 2026 require a generative-AI declaration, and an examiner who
opens `git log` before the viva will find this in ten seconds.

**Nothing here is a problem in itself — undisclosed use is the problem.** The
fix is one paragraph in the front matter, written truthfully.

**Draft, to be checked against the institution's actual policy and edited to
match what really happened:**

> **Declaration on the use of generative AI.** An AI coding assistant (Claude,
> Anthropic) was used throughout this project for drafting and reviewing code,
> drafting and editing prose, and summarising literature. Its use is visible in
> the repository: `CLAUDE.md` records the working rules it was given, and
> commits it contributed to carry a `Co-Authored-By` trailer. The research
> design, the experimental decisions, the interpretation of results and the
> claims made in this thesis are my own. Every number reported regenerates from
> `data/carr.sqlite` via `scripts/results.py` under a fixed seed, and the
> analysis code is covered by 132 tests; I have verified the figures I report
> against that output rather than against any generated text. Literature claims
> flagged in §15 as resting on summaries rather than full reads are marked as
> such.

**Three rules for finishing it.**

1. **Do not understate.** "Used for minor formatting" would be false and is
   checkable against the commit log. Overstating your own independence is the
   only version of this that damages you.
2. **Say what you verified yourself**, because that is the part an examiner
   actually cares about. The reproducibility story here is strong: fixed seeds,
   a regenerating results script, 132 tests, raw responses stored so anything
   can be re-graded.
3. **Check the wording against the real policy.** Some departments require a
   specific form of words or an appendix listing prompts; this draft is a
   starting point, not a substitute for reading the regulations.

**Expect to be asked to explain any part of the code on demand.** That is the
fair test, and it is what [docs/learn/](docs/learn/00-index.md) Part 3 exists to
prepare — every module, and the specific bug each rule prevents.
