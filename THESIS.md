...
| **Spend to date** | $0.00 of $100.00 |
| **Rows in dataset** | 0 of ~5,140 |
...
| ⬜ | 3–4 | **Full grid run** (cheapest configs first) | $25–50 |
...
3. Create an OpenRouter account, generate an API key, load **$10**
...
| DeepSeek-V4-Flash | Cost-efficient reasoner, mid tier | Native: Non-think / Think-High / Think-Max |
...
→ ~10 (model × effort) configurations.
...
### Problems — 514 total
- **HumanEval+** — all 164 (easy anchor, cheap)
- **MBPP+** — 200 sampled, fixed seed (the proposal's 500 buys nothing)
- **LiveCodeBench** — 150, post-cutoff release window, weighted to hard/medium
...
### Budget
~5,140 generations. Non-think ≈ 400 output tokens; think-max on LiveCodeBench ≈ 6–8k. Estimate **$25–50** for a clean pass, leaving headroom for the reruns bugs will force. Replace this guess with measured numbers after the Week-2 pilot.
...
| Risk | Mitigation |
|---|---|
| Budget overrun | Hard cost cap in `experiment.yaml` that aborts the run; cheapest configs first, so a blowout costs the expensive tail not the cheap foundation |
...
| 2026-07-25 | Budget cap $100 | User constraint; grid fitted to ~$25–50 |
...
4. **§6.2** — MBPP 500 → 200; LiveCodeBench 200 → 150 hard/medium-weighted, with rationale
...
1. Update **§1** — stage, next action, spend, row count
2. Tick boxes in **§5**
3. Add a line to the **§1 recent log**
4. Log any decision in **§12**

If a fact lives in two places, this file wins.
